...
public class Translator {
    ...
    private readonly Dictionary<string, List<string>> localizationTerms = new Dictionary<string, List<string>>() {
    ...
    {
        GameEntities.SocketConnection.ChangeLanguage(language);
        CurrentLanguage = language;
    }

    public string GetLocalization(string key)
    {
        localizationTerms.TryGetValue(key, out var terms);

        if (terms == null)
        {
            Debug.LogError($"Language {currentLanguage} does not contain terms for key {key}");
            return string.Empty;
        }
        
        return terms[(int)currentLanguage];
    }
}
